    ...
    <section class="py-20 bg-gray-50 relative overflow-hidden">
        <div class="container mx-auto px-4 relative z-10">
            <div class="max-w-6xl mx-auto">
                <!-- Main Counter Box -->
                <div class="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-2xl p-6 md:p-12 mb-12 md:mb-16 border border-green-50 relative overflow-hidden group" data-aos="fade-up">
                    ...
                    <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8">
                        <div class="text-center md:text-left">
                            <h2 class="text-2xl md:text-3xl font-extrabold text-green-900 mb-1 md:mb-2">Total Jumlah Siswa</h2>
                            <p class="text-gray-500 text-sm md:text-base font-medium mb-4 md:mb-6">Tahun Pelajaran 2023/2024</p>
                            <div class="flex items-baseline justify-center md:justify-start gap-2">
                                <span class="text-6xl md:text-8xl font-black text-green-800 tracking-tighter">139</span>
                                <span class="text-sm md:text-xl font-bold text-yellow-600 uppercase tracking-widest">Siswa Terdaftar</span>
                            </div>
                        </div>
                        <div class="flex-shrink-0 relative">
                            <div class="w-32 h-32 md:w-40 md:h-40 bg-yellow-500 rounded-[2rem] md:rounded-3xl flex items-center justify-center shadow-lg group-hover:rotate-6 transition-transform duration-500 border-4 border-white">
                                <i class="fas fa-users text-4xl md:text-6xl text-green-900"></i>
                            </div>
                            <!-- Small accent -->
                            <div class="absolute -bottom-2 -left-2 md:-bottom-4 md:-left-4 w-10 h-10 md:w-12 md:h-12 bg-green-800 rounded-full flex items-center justify-center text-white text-lg shadow-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div class="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-green-100 hover:border-green-400 hover:shadow-xl hover:-translate-y-1 transition-all duration-300" data-aos="fade-up" data-aos-delay="{{ $idx * 100 }}">
                        <div class="w-10 h-10 md:w-12 md:h-12 bg-green-50 md:bg-green-100 rounded-xl md:rounded-2xl flex items-center justify-center text-green-800 mb-4 md:mb-6">
                            <i class="fas fa-{{ $stat['icon'] }} text-lg md:text-xl"></i>
                        </div>
                        <p class="text-[9px] md:text-[10px] font-black text-green-600/60 uppercase tracking-widest mb-1">{{ $stat['label'] }}</p>
                        <h4 class="text-lg md:text-xl font-bold text-gray-900">{{ $stat['value'] }}</h4>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="max-w-6xl mx-auto">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                    <div data-aos="fade-right">
                        <h2 class="text-2xl md:text-4xl font-extrabold text-green-900 mb-6 md:mb-8 tracking-tight text-center md:text-left">Aktivitas Unggulan & <span class="text-yellow-600">Ekskul</span></h2>
                        <p class="text-gray-600 text-base md:text-lg leading-relaxed mb-8 md:mb-10 font-medium text-center md:text-left">Kami menyediakan berbagai wadah pengembangan diri untuk mengasah minat, bakat, dan karakter setiap siswa.</p>
                        ...
                        <div class="space-y-4 md:space-y-6">
                            ...
                            <div class="flex items-center gap-3 md:gap-4 group">
                                <div class="w-8 h-8 md:w-10 md:h-10 bg-green-50 rounded-full flex items-center justify-center text-green-600 group-hover:bg-green-800 group-hover:text-white transition-colors flex-shrink-0">
                                    <i class="fas fa-check text-[10px] md:text-sm"></i>
                                </div>
                                <span class="text-gray-800 font-bold tracking-tight text-base md:text-lg">{{ $feature }}</span>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    <div class="relative" data-aos="fade-left">
                        <div class="bg-green-900 rounded-[2.5rem] md:rounded-[3rem] overflow-hidden shadow-2xl rotate-2 hover:rotate-0 transition-transform duration-500">
                            <img src="{{ asset('image/pramuka1.jpg') }}" alt="Student Activities" class="w-full h-64 md:h-[500px] object-cover opacity-80">
                        </div>
                        <!-- Absolute floating card -->
                        <div class="absolute -bottom-10 -left-10 bg-white p-8 rounded-3xl shadow-2xl max-w-xs border border-green-50 hidden xl:block">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
                ...
                <div class="group bg-white rounded-[2rem] shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 translate-y-0 hover:-translate-y-2" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
                    <div class="relative overflow-hidden h-56 md:h-64">
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-green-950/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-6 md:p-8">
                            <p class="text-white text-xs md:text-sm font-medium">{{ $desc }}</p>
                        </div>
                    </div>
                    <div class="p-6 md:p-8">
                        <div class="flex justify-between items-center mb-2">
                            <h4 class="text-xl md:text-2xl font-bold text-gray-900 tracking-tight">{{ $title }}</h4>
